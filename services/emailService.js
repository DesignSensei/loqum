...
class EmailService {
  ...
  static async sendInvite(email, inviteUrl, branchName) {
    try {
      await resend.emails.send({
        from: `Loqum <${process.env.EMAIL_FROM}>`,
        to: email,
        ...
        <div style="font-family: 'Fustat', sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">
          ...
          <!-- Body -->
          <div style="padding: 40px;">
            <h2 style="color: #1F2933; font-size: 22px; font-weight: 700; margin: 0 0 12px;">You've Been Invited</h2>
            <p style="color: #1F2933; font-size: 15px; line-height: 1.6; margin: 0 0 32px;">
              You have been invited to join <strong>${branchName}</strong> on Loqum as a team member. Click the button below to set up your account and get started. This invite expires in <strong>48 hours</strong>.
            </p>

            <!-- Button -->
            <div style="text-align: center; margin-bottom: 32px;">
              <a href="${inviteUrl}" style="display: inline-block; background: #08A562; color: #ffffff; font-size: 15px; font-weight: 700; text-decoration: none; padding: 14px 32px; border-radius: 8px; letter-spacing: 0.5px;">
                Accept Invite
              </a>
            </div>
            ...
            <p style="color: #6B7280; font-size: 13px; line-height: 1.6; margin: 0;">
              Or copy and paste this link into your browser:<br/>
              <a href="${inviteUrl}" style="color: #08A562; word-break: break-all;">${inviteUrl}</a>
            </p>
          </div>
      ...
      logger.info(`Invite sent to ${email} for branch: ${branchName}`);
    } catch (error) {
