...
class WalletService {
  ...
  static getCountrySetting(settings, { countryCode, currency }) {
    ...
    return {
      ...
      platformFeeRate:
        countrySetting.platformFeeRate ??
  ...
  static assertWalletIsActive(wallet) {
    ...
    if (wallet.status !== "active") {
      throw new Error(
        `Wallet is ${wallet.status}. Transactions are not allowed.`
      );
    }
  ...
  static assertTransactionBusinessContext({
    ...
    if (normalizedType === "professional_payout") {
      if (!settlementBatch) {
        throw new Error(
          "Professional payout must belong to a ShiftSettlementBatch."
        );
      }
      ...
      if (
        !SETTLEMENT_BATCH_PURPOSES.includes(
          normalizedPurpose
        )
      ) {
        throw new Error(
          "Professional payout must use the weekly professional payout purpose."
        );
      }

      if (
        normalizedPaymentRail !==
        "internal_transfer"
      ) {
        throw new Error(
          "Professional settlement payout must use internal_transfer."
        );
      }
    }
    ...
    if (normalizedType === "platform_fee") {
      if (settlementBatch) {
        throw new Error(
          "Platform-fee movement cannot belong to ShiftSettlementBatch."
        );
      }
      ...
      if (
        !PLATFORM_FEE_PURPOSES.includes(
          normalizedPurpose
        )
      ) {
        throw new Error(
      ...
      if (
        ...
        throw new Error(
          "Platform-fee movement must use internal_transfer."
        );
      }
    }
    ...
    if (
      ...
      normalizedType !== "platform_fee"
    ) {
      throw new Error(
        `${normalizedPurpose} may only be used by a platform_fee transaction.`
      );
    }
  ...
  static async getExistingTransactionByIdempotencyKey(
    idempotencyKey,
    session = null
  ) {
    if (!idempotencyKey) {
  ...
  static assertExistingTransferEntry({
    ...
    if (!transaction) {
      throw new Error(
        "Existing transfer transaction is required."
      );
    }
    ...
    const expectedValues = {
      ...
      shiftOccurrence:
        shiftOccurrence
          ? String(shiftOccurrence)
          : null,

      assignmentCase:
        assignmentCase
          ? String(assignmentCase)
          : null,

      shiftApplication:
        shiftApplication
          ? String(shiftApplication)
          : null,
    ...
    const actualValues = {
      ...
      shift:
        transaction.shift
          ? String(transaction.shift)
          : null,

      shiftOccurrence:
        transaction.shiftOccurrence
          ? String(transaction.shiftOccurrence)
          : null,

      assignmentCase:
        transaction.assignmentCase
          ? String(transaction.assignmentCase)
          : null,

      shiftApplication:
        transaction.shiftApplication
          ? String(transaction.shiftApplication)
          : null,

      settlementBatch:
        transaction.settlementBatch
          ? String(transaction.settlementBatch)
          : null,

      employerRefundBatch:
        transaction.employerRefundBatch
          ? String(transaction.employerRefundBatch)
          : null,

      employerRefundBatchLineId:
        transaction.employerRefundBatchLineId
          ? String(transaction.employerRefundBatchLineId)
          : null,

      dispute:
        transaction.dispute
          ? String(transaction.dispute)
          : null,
    };
    ...
    for (
      ...
      if (
        ...
        throw new Error(
          `Existing transfer transaction does not match the requested ${field}.`
        );
      }
    }
    ...
    if (!transaction.groupReference) {
      throw new Error(
        "Existing transfer transaction has no group reference."
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Existing transfer transaction is ${transaction.status} and requires reconciliation.`
      );
    }
  ...
  static async createEmployerWalletIfMissing(
    employerProfile,
    options = {}
  ) {
    if (!employerProfile) {
      throw new Error(
        "Employer profile is required to create employer wallet."
      );
    }
    ...
    const countrySetting =
      WalletService.getCountrySetting(
        settings,
        {
          countryCode:
            employerProfile.countryCode ||
            settings.defaultCountryCode,

          currency:
            employerProfile.currency ||
  ...
  static async createProfessionalWalletIfMissing(
    professionalProfile,
    options = {}
  ) {
    if (!professionalProfile) {
      throw new Error(
        "Professional profile is required to create professional wallet."
      );
    }
    ...
    const countrySetting =
      WalletService.getCountrySetting(
        ...
        {
          countryCode:
            professionalProfile.countryCode ||
            settings.defaultCountryCode,

          currency:
            professionalProfile.currency ||
            settings.defaultCurrency,
        }
      );
  ...
  static async createEscrowWallet(
    {
      countryCode,
      currency,
    } = {},
  ...
  static async createPlatformWallet(
    {
      countryCode,
      currency,
    } = {},
  ...
  static async getEmployerWallet(
    ...
    if (!employerProfileId) {
      throw new Error(
        "Employer profile ID is required."
      );
    }
  ...
  static async getProfessionalWallet(
    ...
    if (!professionalProfileId) {
      throw new Error(
        "Professional profile ID is required."
      );
    }
  ...
  static async applyWalletMovement({
    ...
    WalletService.assertWalletIsActive(
      wallet
    );
    ...
    if (
      ...
      throw new Error(
        "Transaction direction must be either credit or debit."
      );
    }
    ...
    const balanceBefore =
      WalletService.getBalanceSnapshot(
        wallet
      );

    const delta = {
      availableBalance:
        money.normalizeSignedMinorUnitAmount(
    ...
    const balanceAfter = {
      availableBalance:
        money.normalizeMinorUnitAmount(
          rawBalanceAfter.availableBalance,
    ...
    const transactionData = {
      reference:
        reference ||
        WalletService.getTransactionReference(
          type
        ),
  ...
  static async createPendingExternalCredit(
    ...
    return WalletService.runWithOptionalTransaction(
      options,
      async (session) => {
        if (!idempotencyKey) {
          throw new Error(
            "Idempotency key is required for a pending external credit."
          );
        }

        const existingTransaction =
          await WalletService.getExistingTransactionByIdempotencyKey(
            idempotencyKey,
            session
          );
        ...
        if (!cleanProvider) {
          throw new Error(
            "Provider is required for a pending external credit."
          );
        }

        if (!cleanPaymentRail) {
          throw new Error(
            "Payment rail is required for a pending external credit."
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Paystack payment rails must use paystack as provider."
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Paystack reference is required for a Paystack external credit."
          );
        }
        ...
        if (
          ...
          if (
            ...
            throw new Error(
              "Paystack DVA can only credit an employer wallet."
            );
          }
          ...
          if (!dva) {
            throw new Error(
              "DVA reference is required for a Paystack DVA credit."
            );
          }
        }
  ...
  static async markPendingExternalCreditFailed(
    ...
    return WalletService.runWithOptionalTransaction(
      ...
      async (session) => {
        ...
        if (!transaction) {
          throw new Error(
            "Pending external credit transaction not found."
          );
        }
        ...
        if (
          ...
          throw new Error(
            `A ${transaction.status} transaction cannot be marked as failed.`
          );
        }
        ...
        transaction.failureReason =
          String(
            failureReason ||
              "External payment initialization failed."
          ).slice(
            0,
            300
          );
    ...
    return WalletService.runWithOptionalTransaction(
      ...
      async (session) => {
        if (
          ...
          throw new Error(
            "Transaction ID or Paystack reference is required."
          );
        }
        ...
        if (!transaction) {
          ...
          if (
            ...
            throw new Error(
              "This external payment is already being processed."
            );
          }

          throw new Error(
            `A ${existingTransaction.status} external credit cannot be completed.`
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Provider fee cannot exceed the transaction amount."
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Net amount cannot exceed the transaction amount."
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Employer wallet external top-up balance limit exceeded."
          );
        }
  ...
  static assertExistingExternalDebitTransaction({
    ...
    if (!transaction) {
      throw new Error(
        "Existing external debit transaction is required."
      );
    }
  ...
  static async createPendingExternalDebit(
    ...
    return WalletService.runWithOptionalTransaction(
      ...
      async (session) => {
        if (!idempotencyKey) {
          throw new Error(
            "Idempotency key is required for a pending external debit."
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Pending external debit currently supports Paystack Transfer only."
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Paystack Transfer must use paystack as provider."
          );
        }

        if (!bankAccount) {
          throw new Error(
            "Bank account is required for a Paystack external debit."
          );
        }

        const normalizedPaystackTransferReference =
          String(
            paystackTransferReference ||
              ""
          )
        ...
        if (existingTransaction) {
          ...
          if (!existingWallet) {
            throw new Error(
              "Existing external debit wallet not found."
            );
          }
        ...
        if (
          ...
          throw new Error(
            "Wallet has insufficient available balance for this external debit."
          );
        }
  ...
  static async markPendingExternalDebitProcessing(
    ...
    return WalletService.runWithOptionalTransaction(
      ...
      async (session) => {
        ...
        if (!transaction) {
          throw new Error(
            "Pending external debit transaction not found."
          );
        }
        ...
        if (
          ...
          throw new Error(
            `A ${transaction.status} external debit cannot enter provider processing.`
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Transaction is not a Paystack external debit instruction."
          );
        }
    ...
    return WalletService.runWithOptionalTransaction(
      ...
      async (session) => {
        ...
        transaction.cancellationReason =
          String(
            cancellationReason ||
              "External Transfer was cancelled before provider submission."
          ).slice(
            0,
            300
          );
    ...
    return WalletService.runWithOptionalTransaction(
      ...
      async (session) => {
        ...
        if (
          ...
          throw new Error(
            "Paystack Transfer code is required to complete an external debit."
          );
        }
    ...
    return WalletService.runWithOptionalTransaction(
      ...
      async (session) => {
        if (
          ...
          throw new Error(
            "Both source and destination wallet IDs are required."
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Debit and credit idempotency keys must be different."
          );
        }
        ...
        if (
          !INTERNAL_PAYMENT_RAILS.includes(
            normalizedPaymentRail
          )
        ) {
          throw new Error(
            "Internal wallet transfers must use an internal payment rail."
          );
        }
        ...
        WalletService.assertSameCountryAndCurrency(
          fromWallet,
          toWallet
        );

        {
          const [
            ...
            await Promise.all([
              WalletService.getExistingTransactionByIdempotencyKey(
                debitIdempotencyKey,
                session
              ),
          ...
          if (
            ...
            if (
              ...
              throw new Error(
                "The transfer has only one persisted ledger entry and requires reconciliation."
              );
            }
            ...
            WalletService.assertExistingTransferEntry({
              ...
              countryCode:
                fromWallet.countryCode,

              currency:
                fromWallet.currency,

              groupReference:
                requestedGroupReference,
            ...
            if (
              String(
                existingDebit.relatedTransaction ||
                  ""
              ) !==
                String(
                  existingCredit._id
              ...
              String(
                existingCredit.relatedTransaction ||
                  ""
              ) !==
                String(
                  existingDebit._id
              ...
              throw new Error(
                "Existing transfer ledger entries are not correctly paired."
              );
            }

            if (
              existingDebit.groupReference !==
              existingCredit.groupReference
            ) {
              throw new Error(
                "Existing transfer ledger entries do not share a group reference."
              );
            }
         ...
         */
        const debitResult =
          await WalletService.applyWalletMovement({
            wallet:
              fromWallet,
        ...
        debitResult.transaction.relatedTransaction =
          creditResult.transaction._id;

        creditResult.transaction.relatedTransaction =
          debitResult.transaction._id;

        debitResult.transaction.status =
          "completed";

        creditResult.transaction.status =
          "completed";
